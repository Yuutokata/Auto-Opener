        ...
        post("/send_message") {
            ...
            try {
                ...
                if (successfulSends > 0) {
                    ...
                    call.respond(HttpStatusCode.OK, mapOf("message" to "Message sent to $successfulSends active session(s)."))
                } else if (sessionsFound > 0) {
                    logger.error("Message sending failed for all $sessionsFound found sessions")
                    call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Failed to send message to any active session."))
                } else {
                ...
                call.respond(
                    HttpStatusCode.InternalServerError, mapOf("error" to (e.message ?: "Unknown error processing message"))
                )
            } finally {
                MDC.clear()
            }
